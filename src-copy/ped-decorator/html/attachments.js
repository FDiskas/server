...
function closeWindow(Window) {
  ...
  if (Window == 'ChooseObject') {
    ...
    document.getElementById('ChooseObject').style.borderBottomRightRadius =
      '1vw';
    document.getElementById('MainWindow').style.borderBottomRightRadius = '1vw';
  }
}
...
function openWindow(Window) {
  if (objectList.length > 0) {
    ...
    if (Window == 'EditObject') {
      document.getElementById('ChooseObject').style.borderBottomRightRadius =
        '0vw';
      document.getElementById('MainWindow').style.borderBottomRightRadius =
        '0vw';
    } else if (Window == 'ChooseObject') {
      document.getElementById('EditObject').style.left = '21vw';
      document.getElementById('EditObject').style.borderBottomLeftRadius =
        '0vw';
...
function updateObjectList() {
  ...
  $('#AttachmentObjectList').html(currentObjectList);
}
...
function updateBoneList() {
  ...
  boneList.map((item, index) => {
    objectList.map((obj) => {
      if (obj.id == editObjectId) {
        if (obj.bone == item.id) {
          currentBoneList +=
            "<div class='ChooseObjectItem SelectedItem' onclick='chooseBone(" +
            item.id +
            ")'>";
        } else {
          currentBoneList +=
            "<div class='ChooseObjectItem' onclick='chooseBone(" +
            item.id +
            ")'>";
        }
      }
  ...
  $('#AttachmentBoneList').html(currentBoneList);
}
