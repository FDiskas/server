...
class Tick {
    ...
    clear(id: string) {
        try {
            ...
            alt.logError(error, id);
        }
    }

    private add(id: string, handle: number) {
        if (this.handlers[id]) throw new Error('id already exists');
...
class EveryTick {
    ...
    register(id: string, handler: () => void, timeout: number = -1, timeoutHandler: () => void = () => {}) {
        alt.log('Register tick');
    }
    ...
    private add(id: string, handle: number) {
        alt.log('Added new tick');
        if (this.handlers[id]) throw new Error('id already exists');
        this.handlers[id] = handle;
    }
}

export const everyTick = new EveryTick();
