...
export class Player extends Entity {
    ...
    static async addWeaponComponent(weapon: WeaponHash, component: number) {
        await network.callback('player:addWeaponComponent', [
            weapon,
            component,
        ]);
    }
    ...
    static async removeWeaponComponent(weapon: WeaponHash, component: number) {
        await network.callback('player:removeWeaponComponent', [
            weapon,
            component,
        ]);
    }
    ...
    static teleportTo(coord: alt.Vector3) {
        let pos = game.getClosestMajorVehicleNode(
            coord.x,
            coord.y,
            0,
            undefined,
