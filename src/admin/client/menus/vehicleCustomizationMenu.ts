    ...
    constructor(parentMenu: AbstractMenu, title: string) {
        ...
        this.menuObject.MenuOpen.on(() => {
            ...
            Enum.getValues(VehicleMod).forEach((mod) =>
                this.modMenus.find((menu) => menu.mod == +mod)?.init(this.vehicle)
            );
        });
    }
}
...
class ModMenu extends AbstractSubMenu {
    ...
    init(vehicle: alt.Vehicle) {
        ...
        if (this.numMods == 0) Menu.lockMenuItem(this.menuItem);
        else {
            for (let _i = 0; _i <= this.numMods; _i++) {
                let item = new UIMenuItem(this.menuItem.Text + ' #' + _i);
                this.addItem(item, () => {
            ...
            Menu.selectItem(
                this.menuObject.MenuItems[game.getVehicleMod(vehicle.scriptID, this.mod) + 1],
                BadgeStyle.Car
            );
        }
    }
