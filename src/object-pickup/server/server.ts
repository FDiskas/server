...
class Pickups {
    ...
    _respawnTime: any;
    _disabled: boolean;
    _pickupSound: { name: any; set: any };
    _pickupColshape: alt.ColshapeCylinder;
    _respawnTimeout: number;
    ...
    static create(
        ...
        respawn = false,
        respawnTime = 30000,
        pickupSound = 'Deliver_Pick_Up',
        pickupSoundSet = 'HUD_FRONTEND_MP_COLLECTABLE_SOUNDS'
    ) {
    ...
    onPickup(player) {
        ...
        if (this.respawn)
            this._respawnTimeout = alt.setTimeout(() => {
                this._disabled = false;
                this.createForPlayer(null);
            }, this.respawnTime);
...
alt.onClient('create', (player, coords) => {
    alt.log('~g~ From client');
    alt.emit('pickups:create', 'trash', 'prop_rub_binbag_05', coords);
});
