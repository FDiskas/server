        ...
        if (args.length > 0) {
            switch (args[0]) {
                case 'help': {
                    ...
                    alt.log(
                        '  * Type "animator goto [dictionary] [animation]" to directly go to the defined animation (string or int).'
                    );
...
const dictList = new NativeUI.UIMenuListItem(
    'Dictionary:',
    'Select the animation dictionary',
    getDictsAsItemCollection()
);
const animList = new NativeUI.UIMenuListItem(
    'Animation:',
    'Select the animation name',
    getAnimsForSelectedDictionaryAsItemCollection()
);
